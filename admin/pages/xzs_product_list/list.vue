      ...
      <unicloud-db ref="udb" :collection="collectionList" :where="where" page-data="replace"
        ...
        <uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection" @selection-change="selectionChange">
          <uni-tr>
            ...
            <uni-th align="center" sortable @sort-change="sortChange($event, 'picurl')">茶品图</uni-th>
          ...
          <uni-tr v-for="(item,index) in data" :key="index">
            ...
            <uni-td align="center">{{item.listId[0].text}}</uni-td>
            <uni-td align="center">
...
<script>
  ...
  export default {
    data() {
      return {
        collectionList: [ db.collection('xzs_product_list').field('title,listId,picurl,orderid,year,weight,pronum,grade,price,checked').getTemp(),db.collection('xzs_product_nav').field('_id, classname as text').getTemp() ],
        query: '',
        where: '',
        orderby: dbOrderBy,
        orderByFieldName: "",
